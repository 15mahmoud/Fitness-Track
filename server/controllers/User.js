...
export const UserRegister = async (req, res, next) => {
  try {
    ...
    const token = jwt.sign({ id: createdUser._id }, "mySuperSecretKey", {
...
export const addWorkout = async (req, res, next) => {
  try {
    ...
    // Calculate calories and add workouts to the database
    await Promise.all(
    ...
    console.error("Error occurred while adding workouts:", err);
    return res.status(500).json({
      message: "An error occurred while adding workouts",
      error: err.message,
    });
  }
};
